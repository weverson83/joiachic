...
class Amasty_Fpc_Model_Fpc extends Mage_Core_Model_Cache
{
    ...
    protected $_discardedBlocks = null;

    public function __construct(array $options = array())
    {
        $configOptions = Mage::app()->getConfig()->getNode('global/amfpc/cache');

        if ($configOptions === false){ // Backwards compatibility
    ...
    protected function _getBackendOptions(array $cacheOptions)
    {
        ...
        {
            $options['options'] = $this->getDbAdapterOptions($options['options']);
        }
    ...
    public function savePage($data, $tags, $lifetime)
    {
        ...
        if (Mage::registry('amabtesting_test')) {
            $variation = Mage::registry('amabtesting_variation');
            $visitor = Mage::getSingleton('amabtesting/visitor');
            $variationId = $visitor->isIgnored() ? 0
                : ($variation ? $variation->getId() : 0);

            $this->_savePage(
                $data, $key . '_' . $variationId, $tags, $lifetime
            );
        ...
        $this->_savePage($data, $key, $tags, $lifetime);
    }
    ...
    public function loadPage($id)
    {
        ...
        if ($content) {
            ...
            $variation = $this->_getAbTestVariation($content);
            if (false !== $variation) {
                return $this->loadPage($id . '_' . $variation);
            }
        }
    ...
    protected function _getAbTestVariation($data)
    {
        ...
        if (!Mage::getSingleton('amfpc/fpc_front')->isModuleEnabled('Amasty_Abtesting'))
            return false;
        ...
        $variation = Mage::getModel('amabtesting/visitor')
            ->getCurrentVariation($data['product_id'])
        ;
    ...
    public function saveBlockCache($name, $content, $tags)
    {
        ...
        if (in_array($name, array(
            Amasty_Fpc_Model_Config::getCookieNoticeBlockName(), 'google_analytics')
        )) {
            if (!trim($content))
                return;
        }
        ...
        if (Mage::helper('core')->isModuleEnabled('Amasty_Optimization')) {
            $helper = Mage::helper('amoptimization/js');
            if ($helper->isFooterJsEnabled()) {
                $content = $helper->removeJs($content);
            }
        }
