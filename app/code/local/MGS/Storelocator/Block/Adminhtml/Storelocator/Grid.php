 ...
 * 
 */
class MGS_Storelocator_Block_Adminhtml_Storelocator_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
     ...
     *
     * @return MGS_Storelocator_Block_Adminhtml_Storelocator_Grid
     */
    protected function _prepareCollection()
    {
        $collection = Mage::getModel('mgs_storelocator/storelocator')->getCollection();
        /* @var $collection MGS_Storelocator_Model_Resource_Storelocator_Collection */
    ...
    protected function _prepareColumns()
    {
        ...
        */
        ...
        $enableDisable = Mage::getModel('mgs_storelocator/enabledisable')->toArray();
        
        $this->addColumn('name', array(
            'header'=>Mage::helper('mgs_storelocator')->__('Store Name'),
            'sortable'=>true,
            'index'=>'name'
        ));
		
        $this->addColumn('country', array(
            'header'=>Mage::helper('mgs_storelocator')->__('Country'),
            'sortable'=>true,
            'index'=>'country',
            'type' => 'country',
        ));
        ...
        $this->addColumn('state', array(
            'header'=>Mage::helper('mgs_storelocator')->__('State'),
            'sortable'=>true,
            'index'=>'state'
        ));
        
        $this->addColumn('city', array(
            'header'=>Mage::helper('mgs_storelocator')->__('City'),
            'sortable'=>true,
            'index'=>'city'
        ));
        
        $this->addColumn('zipcode', array(
            'header'=>Mage::helper('mgs_storelocator')->__('Zipcode'),
            'sortable'=>true,
            'index'=>'zipcode'
        ));
        ...
        if(!$this->_isExport) {
            ...
            if (!Mage::app()->isSingleStoreMode()) {
                $this->addColumn('store_id', array(
                    'header'        => Mage::helper('mgs_storelocator')->__('Store View'),
                    'index'         => 'store_id',
                    'type'          => 'store',
                    'store_all'     => true,
                    'store_view'    => true,
            ...
            $this->addColumn('store_id', array(
            'header'=> Mage::helper('mgs_storelocator')->__('Store View [ admin - All Store Views ]'),
            'index' => 'store_id',
            'renderer'  => 'MGS_Storelocator_Block_Adminhtml_Storelocator_Renderer_Store',
            ));
        }
        
        $this->addColumn('status', array(
            'header'=>Mage::helper('mgs_storelocator')->__('Status'),
            'index'     => 'status',
            'width'=>'100px',
            'type'      => 'options',
       ...
       $this->addColumn('action',
        array(
            'header'    => Mage::helper('mgs_storelocator')->__('Action'),
            'type'      => 'action',
            'getter'    => 'getId',
            'actions'   => array(
                array(
                    'caption' => Mage::helper('mgs_storelocator')->__('Edit'),
                    'url'     => array('base'=> '*/*/edit'),
       ...
       if($this->_isExport) {
        $this->addColumn('street_address', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Street Address'),
       ...
       if($this->_isExport) {
        $this->addColumn('phone', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Phone'),
             'index'=>'phone'
         ));
       } 
       ...
       if($this->_isExport) {
        $this->addColumn('fax', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Fax'),
             'index'=>'fax'
         ));
       } 
       ...
       if($this->_isExport) {
        $this->addColumn('url', array(
             'header'=>Mage::helper('mgs_storelocator')->__('URL'),
             'index'=>'url'
         ));
       } 
       ...
       if($this->_isExport) {
        $this->addColumn('email', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Email'),
             'index'=>'email'
         ));
       } 
       ...
       if($this->_isExport) {
        $this->addColumn('store_logo', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Store Logo'),
             'index'=>'store_logo'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('trading_hours', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Trading Hours'),
             'index'=>'trading_hours'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('radius', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Radius'),
             'index'=>'radius'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('latitude', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Latitude'),
             'index'=>'latitude'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('longitude', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Longitude'),
             'index'=>'longitude'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('zoom_level', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Zoom Level'),
             'index'=>'zoom_level'
         ));
       }
       ...
       if($this->_isExport) {
        $this->addColumn('description', array(
             'header'=>Mage::helper('mgs_storelocator')->__('Description'),
             'index'=>'description'
         ));
       }
       
      //Import Export functionality
      $this->addExportType('*/*/exportCsv', Mage::helper('mgs_storelocator')->__('CSV'));
      $this->addExportType('*/*/exportXml', Mage::helper('mgs_storelocator')->__('XML'));
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('delete', array(
             'label'    => Mage::helper('mgs_storelocator')->__('Delete'),
             'url'      => $this->getUrl('*/*/massDelete'),
             'confirm'  => Mage::helper('mgs_storelocator')->__('Are you sure?')
        ));
        
        /**
         * Get the enable disable drop down array
         */
        $enableDisable = Mage::getModel('mgs_storelocator/enabledisable')->toOptionArray();
        
        $this->getMassactionBlock()->addItem('status', array(
             'label'=> Mage::helper('mgs_storelocator')->__('Change status'),
