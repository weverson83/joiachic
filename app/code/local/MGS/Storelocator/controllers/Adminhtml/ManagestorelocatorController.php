 ...
 */
class MGS_Storelocator_Adminhtml_ManagestorelocatorController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Init actions
     *
    ...
    protected function _initAction()
    {
        ...
        $this->loadLayout()
            ->_setActiveMenu('mgs_storelocator/mgs_manage_storelocator')
            ->_addBreadcrumb(Mage::helper('mgs_storelocator')->__('Store Locator'), Mage::helper('mgs_storelocator')->__('Store Locator'))
        ;
        return $this;
    }
    ...
    public function gridAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('mgs_storelocator/adminhtml_storelocator_grid')->toHtml()
        );
    }
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('mgs_storelocator/adminhtml_storelocator_grid')
            ->getCsv();
            ...
            if (!empty($storelocatorIds)) {
                try {
                    foreach ($storelocatorIds as $storelocatorId) {
                        $storelocator = Mage::getSingleton('mgs_storelocator/storelocator')->load($storelocatorId);
                        //delete image file
                        if($storelocator->getStoreLogo()) {
                            $oldFileTargetPath = Mage::getBaseDir('media') . DS . 'mgs_storelocator' . DS . $storelocator->getStoreLogo();
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($storelocatorIds)) {
            ...
            try {
                foreach ($storelocatorIds as $storelocatorId) {
                    Mage::getSingleton('mgs_storelocator/storelocator')
                    ->load($storelocatorId)
    ...
    public function editAction()
    {
        ...
        if ($id) {
            ...
            if (! $model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('mgs_storelocator')->__('This store no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        $this->_initAction()
            ->_addBreadcrumb(
                $id ? Mage::helper('mgs_storelocator')->__('Edit Store')
                    : Mage::helper('mgs_storelocator')->__('New Store'),
                $id ? Mage::helper('mgs_storelocator')->__('Edit Store')
                    : Mage::helper('mgs_storelocator')->__('New Store'));
        
         $this->_addContent($this->getLayout()->createBlock('mgs_storelocator/adminhtml_storelocator_edit'))
                 ->_addLeft($this->getLayout()->createBlock('mgs_storelocator/adminhtml_storelocator_edit_tabs'));
    ...
    public function uploadFile($targetPath)
    {
        try {      
            ...
            if (!$result) {
                 Mage::throwException( Mage::helper('mgs_storelocator')->__('Cannot upload file.') );
            }
            return $result;
        } catch (Exception $e) {
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($id) {
                $model->load($id);
                 if (!$model->getId()) {
                    $session->addError(Mage::helper('mgs_storelocator')->__('This store no longer exists.'));
                    $this->_redirect('*/*/');
                    return;
                 }
            }
            ...
            if(isset($_FILES['store_logo']['name']) && $_FILES['store_logo']['name'] != '') {
                $targetPath = Mage::getBaseDir('media') . DS . 'mgs_storelocator' . DS; //desitnation directory  
                //upload file
                ...
                //delete old file
                if ($id) {
                    $oldFileTargetPath = Mage::getBaseDir('media') . DS . 'mgs_storelocator' . DS . $model->getStoreLogo();
                    $this->deleteFile($oldFileTargetPath);
                }
            } else {  
            ...
            try {
                ...
                $session->addSuccess(Mage::helper('mgs_storelocator')->__('The store has been saved.'));
                ...
                Mage::helper('mgs_storelocator')->__('An error occurred while saving the store.'));
            }
            $session->setStorelocatorData($data);
            $this->_redirect('*/*/edit', array('storelocator_id' => $this->getRequest()->getParam('storelocator_id')));
            return;
    ...
    public function validate($data)
    {
        ...
        $storelocator = Mage::getModel('mgs_storelocator/storelocator');
        ...
        if($storelocator->storeExists($storeName, $storelocatorId)) {
            $errors[] = Mage::helper('mgs_storelocator')->__('Store name already exists');
        }
        ...
        if ($_FILES['store_logo']['error'] == 0) {
            ...
            if($fileExtension){
                if(!in_array($fileExtension, $allowedExtensions)) {
                    $errors[] = Mage::helper('mgs_storelocator')->__('Allowed image extension are jpg, jpeg, gif, png');
                }
            }
        }
    ...
    public function deleteAction()
    {
        ...
        if ($id = $this->getRequest()->getParam('storelocator_id')) {
            try {
                ...
                // display success message
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('mgs_storelocator')->__('The store has been deleted.'));
                // go to grid
                $this->_redirect('*/*/');
                return;
        ...
        // display error message
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mgs_storelocator')->__('Unable to find a store to delete.'));
        // go to grid
        $this->_redirect('*/*/');
    }
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'new':
            case 'save':
                return Mage::getSingleton('admin/session')->isAllowed('mgs_storelocator/mgs_manage_storelocator/save');
                break;
            case 'massDelete':
                return Mage::getSingleton('admin/session')->isAllowed('mgs_storelocator/mgs_manage_storelocator/delete');
                break;
            case 'delete':
                return Mage::getSingleton('admin/session')->isAllowed('mgs_storelocator/mgs_manage_storelocator/delete');
                break;
             ...
             case 'exportCsv':
              case 'exportXml':
                return Mage::getSingleton('admin/session')->isAllowed('mgs_storelocator/import_export/export');
                break;
            default:
                return Mage::getSingleton('admin/session')->isAllowed('mgs_storelocator/mgs_manage_storelocator');
                break;
        }
    }
    ...
    {
        if (Mage::helper('mgs_storelocator/image')->flushImagesCache()) {
