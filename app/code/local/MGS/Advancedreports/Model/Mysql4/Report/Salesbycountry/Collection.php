...
class MGS_Advancedreports_Model_Mysql4_Report_Salesbycountry_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
    ...
    public function __construct() {
        ...
        $this->_resource = Mage::getResourceModel('advancedreports/salesbycountry')->init('advancedreports/salesbycountry');
     ...
     *
     * @return MGS_Advancedreports_Model_Mysql4_Report_Salesbycountry_Collection
     */
    protected function _getSelectedColumns() {
        ...
        if ($this->isTotals()) {
            ...
            $this->_selectedColumns = array(
                ...
                'total_item_count' => 'IFNULL(SUM(total_item_count), 0)',
                'grand_total' => 'IFNULL(SUM(grand_total), 0)'
            );
        }
    ...
    protected function _initSelect() {
        ...
        if (!$this->isTotals()) {
            $this->getSelect()->order('total_item_count DESC');
            $this->getSelect()->join(Mage::getSingleton('core/resource')->getTableName('sales_flat_order_address'), Mage::getSingleton('core/resource')->getTableName('sales_flat_order') . '.billing_address_id = ' . Mage::getSingleton('core/resource')->getTableName('sales_flat_order_address') . '.entity_id', array('country_id'));
            $this->getSelect()->group(Mage::getSingleton('core/resource')->getTableName('sales_flat_order_address') . '.country_id');
        }
        return $this;
    }
     ...
     * @return boolean|MGS_Advancedreports_Model_Mysql4_Report_Salesbycountry_Collection
     */
