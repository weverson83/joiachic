 ...
 * @license    MIT
 */
class Trezo_Cielo_Model_Webservice_Cielo
{
    /**
    ...
    {
        if (strlen($params['numero']) === 0 || strlen($params['chave']) === 0) {
            throw new Exception('Número/Chave inválidos.');
        }

        if (strlen($params['numero']) > 20) {
            throw new Exception('Número inválido.');
        }

        if (strlen($params['chave']) > 100) {
        ...
        $this->_numero = substr($params['numero'], 0, 20);
        $this->_chave = substr($params['chave'], 0, 100);
    }
    ...
    {
        if (preg_match('/(visa|mastercard|amex|diners|elo)/i', $_bandeira)) {
   ...
   /**
    ...
    private function _enviaChamada()
    {
		...
		curl_setopt($_curl, CURLOPT_TIMEOUT, 90);
