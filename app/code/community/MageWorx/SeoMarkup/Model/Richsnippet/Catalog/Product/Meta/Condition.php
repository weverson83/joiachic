...
class MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Meta_Condition extends MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Abstract
{
    ...
    {
        $conditionValue = Mage::helper('mageworx_seomarkup')->getConditionValue($this->_product);

        if ($conditionValue) {
            $node->innertext = $node->innertext . '<meta itemprop="itemCondition" content="'. $conditionValue. '">' . "\n";
            return true;
        }
        return false;
    }
