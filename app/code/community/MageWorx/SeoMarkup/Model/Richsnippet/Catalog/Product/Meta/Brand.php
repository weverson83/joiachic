 ...
 */
class MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Meta_Brand extends MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Abstract
{
    protected function _addAttributeForNodes(simple_html_dom_node $node)
    {
        $brandValue = Mage::helper('mageworx_seomarkup')->getBrandValue($this->_product);
        if ($brandValue) {
            $node->innertext = $node->innertext . '<meta itemprop="brand" content="'. $brandValue. '">' . "\n";
            return true;
        }
        return false;
    }
