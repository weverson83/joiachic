 ...
 */
class MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Meta_Sku extends MageWorx_SeoMarkup_Model_Richsnippet_Catalog_Product_Abstract
{
    protected function _addAttributeForNodes(simple_html_dom_node $node)
    {
        $sku = Mage::helper('mageworx_seomarkup')->getSkuValue($this->_product);
        if ($sku) {
            $node->innertext = $node->innertext . '<meta itemprop="sku" content="'. $sku. '">' . "\n";
            return true;
        }
        return false;
    }
