...
var AM = AM || {};
AM.MediabrowserUtility = {
    openDialog: function(url, id, width, height, title) {
        if (typeof Windows === 'undefined') return;
        id = id || 'browser_window';
        if ($(id)){
            Windows.focus(id);
            return;
        }
        ...
        new Ajax.Updater('modal_dialog_message', url + '?windowId=' + id, {evalScripts: true});
    },
...
Mediabrowser.prototype = {
    ...
    initialize: function (setup) {
        this.windowId = setup.windowId || 'browser_window',
        ...
        this.onInsertCallback = setup.onInsertCallback;
        this.onInsertCallbackParams = setup.onInsertCallbackParams;
                ...
                try {
                    ...
                    if (targetEl){
                        if (targetEl.tagName.toLowerCase() == 'input') {
                            ...
                            if (fireEvent) fireEvent(targetEl, 'change');
                        } else {
                    ...
                    if (this.onInsertCallback){
                        var names = this.onInsertCallback.split('.');
                        if (names.length == 1){
                            window[names[0]](transport.responseText, this.onInsertCallbackParams);
                        }else if (names.length == 2){
                            window[names[0]][names[1]](transport.responseText, this.onInsertCallbackParams);
                        }
                    }
                } catch (e) {
                    alert(e.message);
